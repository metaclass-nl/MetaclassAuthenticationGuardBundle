    ...
    public function checkAuthentication($justFailed=false) 
    {
        ...
        if ($this->isUserReleasedOnAddress) {
            ...
            if ($this->failureCountForUserByCookie > $this->limitPerUserName) {
                $error = new UsernameBlockedForCookieException("Username '$this->username' is blocked for cookie '$this->cookieToken': $this->failureCountForUserByCookie attempts failed");
            }
        } else {
